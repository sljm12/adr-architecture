...
async function mockApi(page: Page, documents: typeof first[]) {
  await page.route('**/api/diagrams', route => route.fulfill({ contentType: 'application/json', body: JSON.stringify(documents.map(({ components, relationships, trashedAt, ...summary }) => summary)) }));
