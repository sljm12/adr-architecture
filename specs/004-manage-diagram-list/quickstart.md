...
```bash
npm test -- shared/tests/validation.test.ts frontend/tests/saved-diagram-list.test.tsx frontend/tests/saved-diagram-store.test.ts backend/tests/diagrams.test.ts backend/tests/recovery.test.ts
```

Expected result: summary validation accepts `createdAt`; list derivation and panel states cover
name/date filtering, inclusive boundaries, sorting, duplicate names, clear/reset, delete failure,
and stable UUID behavior; API/repository tests cover active-to-trashed and restore preservation.
