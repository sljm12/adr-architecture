...
export class DiagramRepository {
  ...
  removeComponent(diagramId: string, componentId: string): { document: DiagramDocument; relationshipCount: number } | { conflict: true; relationshipCount: number } | undefined {
    ...
    const updated = { ...document, updatedAt: new Date().toISOString(), components: document.components.filter(c => c.id !== componentId) };
    this.documents.set(diagramId, clone(updated)); return { document: clone(updated), relationshipCount: 0 };
  }
