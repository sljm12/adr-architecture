...
export const components=pgTable('components',{id:uuid('id').primaryKey(),diagramId:uuid('diagram_id').notNull().references(()=>diagrams.id),name:varchar('name',{length:200}).notNull(),description:text('description'),type:varchar('type',{length:100}),x:doublePrecision('x').notNull(),y:doublePrecision('y').notNull(),width:doublePrecision('width').notNull().default(180),height:doublePrecision('height').notNull().default(72),createdAt:timestamp('created_at',{withTimezone:true}).notNull(),updatedAt:timestamp('updated_at',{withTimezone:true}).notNull()});
