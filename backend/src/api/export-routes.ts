...
export function registerExportRoutes(app: FastifyInstance, service: ExportService): void {
  app.get<{ Params: { diagramId: string } }>('/diagrams/:diagramId/export/mermaid', async (request, reply) => {
    try { const result = service.exportMermaid(request.params.diagramId); return reply.type('text/vnd.mermaid; charset=utf-8').header('content-disposition', `attachment; filename="${result.filename}"`).send(result.source); }
