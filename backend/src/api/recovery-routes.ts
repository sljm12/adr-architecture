...
export function registerRecoveryRoutes(app: FastifyInstance, repository: DiagramRepositoryLike, service: DiagramService) {
  ...
  app.get('/diagrams/trash', async () => (await repository.listTrash()).map(document => ({ id: document.id, name: document.name, status: document.status, updatedAt: document.updatedAt })));
  ...
  app.post<{ Params: { diagramId: string } }>('/diagrams/:diagramId/restore', async (request, reply) => { try { return await service.restore(request.params.diagramId); } catch (error) { if (error instanceof DiagramConflictError) return reply.code(409).send({ message: error.message }); return missing(error, reply); } });
}
