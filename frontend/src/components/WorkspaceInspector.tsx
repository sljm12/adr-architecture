...
export function WorkspaceInspector({ mode, selection, onClose }: { mode: InspectorMode; selection: CanvasSelection; onClose: () => void }) {
  ...
  return <aside className="workspace-inspector" aria-label="Diagram inspector"><div className="inspector-header"><div><span className="eyebrow">Inspector</span><h2>{heading}</h2></div><button className="icon-button" type="button" onClick={onClose} aria-label="Close inspector">×</button></div>
    {mode === 'adr' && <><AdrList diagramId={document.id} /><AdrEditor /></>}
