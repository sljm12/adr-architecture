...
export function DiagramWorkspace() {
  ...
  const selectCanvasItem = useCallback((next: CanvasSelection) => { setSelection(current => sameSelection(current, next) ? current : next); updateSelectedComponentIds(next?.kind === 'components' ? next.ids : []); setInspectorMode(null); }, [updateSelectedComponentIds]);
  const selectLinkedComponent = useCallback((componentId: string) => { setSelectedComponentIds([]); setSelection({ kind: 'component', id: componentId }); setInspectorMode(null); }, []);
