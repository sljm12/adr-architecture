...
export function DiagramWorkspace() {
  ...
  const [confirmNew, setConfirmNew] = useState(false); const [pendingDiagramId, setPendingDiagramId] = useState<string | null>(null); const [libraryOpen, setLibraryOpen] = useState(true); const [inspectorMode, setInspectorMode] = useState<InspectorMode>(null); const [selection, setSelection] = useState<CanvasSelection>(null); const [selectedComponentIds, setSelectedComponentIds] = useState<string[]>([]); const [canvasEpoch, setCanvasEpoch] = useState(0);
  ...
  const startFreshDiagram = useCallback(() => { clearCanvasSelection(); setInspectorMode(null); startNew(); }, [clearCanvasSelection, startNew]);
  ...
  const load = async (id: string) => { if (await loadSavedDocument(id)) { clearCanvasSelection(); setCanvasEpoch(value => value + 1); } };
  ...
  const selectCanvasItem = useCallback((next: CanvasSelection) => { setSelection(current => sameSelection(current, next) ? current : next); updateSelectedComponentIds(next?.kind === 'components' ? next.ids : next?.kind === 'component' ? [next.id] : []); setInspectorMode(null); }, [updateSelectedComponentIds]);
  const selectLinkedComponent = useCallback((componentId: string) => { setSelectedComponentIds([componentId]); setSelection({ kind: 'component', id: componentId }); setInspectorMode(null); }, []);
  const selectLinkedRelationship = useCallback((relationshipId: string) => { setSelectedComponentIds([]); setSelection({ kind: 'relationship', id: relationshipId }); setInspectorMode(null); }, []);
  const openLinkedAdr = useCallback((adrId: string) => { clearCanvasSelection(); setInspectorMode('adr'); void selectAdr(adrId); }, [clearCanvasSelection, selectAdr]);
  const openInspector = useCallback((mode: Exclude<InspectorMode, null>) => { setSelection(null); if (mode !== 'group') setSelectedComponentIds([]); setInspectorMode(mode); }, []);
  const closeInspector = useCallback(() => { clearCanvasSelection(); setInspectorMode(null); }, [clearCanvasSelection]);
