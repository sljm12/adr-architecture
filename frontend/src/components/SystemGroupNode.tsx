...
export function SystemGroupNode({ data }: { data: SystemGroupNodeData }) {
  return <div className={`system-group-node${data.isSelected ? ' is-selected' : ''}`} role="group" tabIndex={0} aria-selected={data.isSelected ? 'true' : 'false'} aria-label={`System group ${data.label}, ${data.memberCount} Software System members`}>
