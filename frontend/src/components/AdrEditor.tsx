...
export function AdrEditor() {
  ...
  return <form className="adr-editor" onSubmit={event => { event.preventDefault(); void save(); }} aria-labelledby="adr-editor-heading">
    ...
    {(['title', 'context', 'decision', 'consequences'] as const).map(name => <label key={name} htmlFor={`adr-${name}`}>{name[0].toUpperCase() + name.slice(1)}<span className="required-mark"> required</span><textarea id={`adr-${name}`} rows={name === 'title' ? 2 : 4} {...field(name)} aria-invalid={Boolean(fieldErrors[name])} aria-describedby={messageId(name)} />{fieldErrors[name] && <span className="field-error" id={`${name}-error`}>{fieldErrors[name]}</span>}</label>)}
