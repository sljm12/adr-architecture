...
export const useDiagramStore = create<State>((set, get) => ({
  ...
  removeRelationship: relationshipId => { const d = get().document; if (!d || !d.relationships.some(relationship => relationship.id === relationshipId)) return; get().update(x => ({ ...x, relationships: x.relationships.filter(relationship => relationship.id !== relationshipId) })); },
